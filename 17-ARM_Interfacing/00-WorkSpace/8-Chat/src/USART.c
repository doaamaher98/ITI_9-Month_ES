...
USART_tstr_Registers_t * const volatile USART[3] = {
		/* USART1 */ [USART1] = {0x40011000},
		/* USART2 */ [USART2] = {0x40004400},
		/* USART6 */ [USART6] = {0x40114000},
};
...
/************************************************************************************/
USART_tenuErrorStatus USART_enuInit(USART_tstrConfig_t *USART_Cfg)
{
	...
	for (LocalIterator=0; LocalIterator <NUMBER_OF_USART_CHANNELS ; LocalIterator++)
	{
		...
		if ((USART_Cfg[LocalIterator].USART_Channel == USART_enuUSART1) || (USART_Cfg[LocalIterator].USART_Channel == USART_enuUSART1))
		{
			...
			GPIO_enuSetAlternateFunction(USART_Cfg[LocalIterator].USART_Pins.USART_TX.GPIO_Port,USART_Cfg[LocalIterator].USART_Pins.USART_TX.GPIO_Pin,GPIO_enu_AF_HIGH_7);
			GPIO_enuSetAlternateFunction(USART_Cfg[LocalIterator].USART_Pins.USART_RX.GPIO_Port,USART_Cfg[LocalIterator].USART_Pins.USART_RX.GPIO_Pin,GPIO_enu_AF_HIGH_7);
		}

		else if (USART_Cfg[LocalIterator].USART_Channel == USART_enuUSART6)
		{
			// Set the Alternate Function : 8 For TX and RX Ports/Pins
			GPIO_enuSetAlternateFunction(USART_Cfg[LocalIterator].USART_Pins.USART_TX.GPIO_Port,USART_Cfg[LocalIterator].USART_Pins.USART_TX.GPIO_Pin,GPIO_enu_AF_HIGH_8);
		}
		...
		if (USART_Cfg[LocalIterator].USART_OverSampling == USART_enuOverSampling_DIV_8)
		{
			LocalBuffer |= USART_CR1_OVERSAMPLE_DIV_8;
		}
		else if (USART_Cfg[LocalIterator].USART_OverSampling == USART_enuOverSampling_DIV_16)
		{
		...
		NVIC_enuEnableInterrupt(USART_enuUSART1);
		...
		/* Testing */
		USART[USART_Cfg[LocalIterator].USART_Channel]->USART_SR =0;
	}
