			...
			else if ((Cpy_u8Pin >= GPIO_enuPin_8) && (Cpy_u8Pin <= GPIO_enuPin_15))
			{
				for (LocalIterator = 8 ; LocalIterator < 16 ; LocalIterator++)
				{
					...
					if ((Cpy_u8Pin >> LocalIterator) && 1)
					{
						...
						{
						case GPIO_enuPort_A:
							LocalHighBuffer = GPIO[0]->GPIO_AFRH;
							LocalHighBuffer |= (Cpy_u8AF << (4 * (LocalIterator-8)));
							GPIO[0]->GPIO_AFRH = LocalHighBuffer;
							break;

						case GPIO_enuPort_B:
							LocalHighBuffer = GPIO[1]->GPIO_AFRH;
							LocalHighBuffer |= (Cpy_u8AF << (4 * (LocalIterator-8)));
							GPIO[1]->GPIO_AFRH = LocalHighBuffer;
							break;

						case GPIO_enuPort_C:
							LocalHighBuffer = GPIO[2]->GPIO_AFRH;
							LocalHighBuffer |= (Cpy_u8AF << (4 * (LocalIterator-8)));
							GPIO[2]->GPIO_AFRH = LocalHighBuffer;
							break;

						case GPIO_enuPort_D:
							LocalHighBuffer = GPIO[3]->GPIO_AFRH;
							LocalHighBuffer |= (Cpy_u8AF << (4 * (LocalIterator-8)));
							GPIO[3]->GPIO_AFRH = LocalHighBuffer;
							break;

						case GPIO_enuPort_E:
							LocalHighBuffer = GPIO[4]->GPIO_AFRH;
							LocalHighBuffer |= (Cpy_u8AF << (4 * (LocalIterator-8)));
							GPIO[4]->GPIO_AFRH = LocalHighBuffer;
							break;

						case GPIO_enuPort_H:
							LocalHighBuffer = GPIO[5]->GPIO_AFRH;
							LocalHighBuffer |= (Cpy_u8AF << (4 * (LocalIterator-8)));
							GPIO[5]->GPIO_AFRH = LocalHighBuffer;
							break;
						}
					}
				}
