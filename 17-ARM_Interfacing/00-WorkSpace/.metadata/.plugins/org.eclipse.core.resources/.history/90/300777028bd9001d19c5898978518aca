...
NVIC_tstrRegisters_t * const volatile NVIC = ((NVIC_tstrRegisters_t *) 0xE000E100);
...
NVIC_tenuErrorStatus NVIC_enuEnableInterrupt (u8 Cpy_u8InterruptNum)
{
	...
	if (Cpy_u8InterruptNum > NVIC_IRQ_NUMBERS)
	{
		LocalErrorStatus = NVIC_enuInvalidIRQ;
	}
	else
	{
		/* Clear previous pendings before enabling the wanted interrupt */
		NVIC->NVIC_ICPR[Cpy_u8InterruptNum/32] |= (1<< (Cpy_u8InterruptNum % 32));

		/* Enabling the Wanted interrupt */
		NVIC->NVIC_ISER[Cpy_u8InterruptNum/32] |= (1<< (Cpy_u8InterruptNum % 32));
