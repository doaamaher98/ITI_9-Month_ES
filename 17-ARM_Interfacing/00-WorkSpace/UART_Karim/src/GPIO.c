...
GPIO_tenuErrorStatus GPIO_GetPin(GPIO_Cfg_t* Add_GPIO_Cfg, u8 Pin_No, u16 * Cpy_var)
{
	...
	* Cpy_var = (loc_GPIO->GPIO_IDR >> Pin_No) & 1;
...
GPIO_tenuErrorStatus GPIO_SetAFPORT(void* PORT, u16 Cpy_pin_No, u8 AF_no)
{
	GPIO_tenuErrorStatus loc_Error_Status = GPIO_enuOK;
	GPIO_t * loc_GPIO = (GPIO_t*)(PORT);
	u32 loc_temp;
	u8 loc_iterator;
	if(AF_no > 15)
	{
	...
	{
		if(Cpy_pin_No < GPIO_PIN_8)
		{
			for(loc_iterator=0;loc_iterator<8;loc_iterator++)
			{
				if(((Cpy_pin_No >> loc_iterator) & 1) == 1)
				{
					loc_temp = loc_GPIO->GPIO_AFRL;
					loc_temp  |= (AF_no<<(4*loc_iterator));
					loc_GPIO->GPIO_AFRL = loc_temp;
		...
		else if(Cpy_pin_No >= GPIO_PIN_8)
		{
			for(loc_iterator=8;loc_iterator<16;loc_iterator++)
			{
				if(((Cpy_pin_No >> loc_iterator) & 1) == 1)
				{
					loc_temp = loc_GPIO->GPIO_AFRH;
					loc_temp  |= (AF_no<<(4*(loc_iterator-8)));
					loc_GPIO->GPIO_AFRH = loc_temp;
